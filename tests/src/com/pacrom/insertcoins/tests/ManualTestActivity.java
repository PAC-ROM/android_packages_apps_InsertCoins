...
package com.pacrom.insertcoins.tests;

import com.pacrom.insertcoins.InsertCoins;
...
public class ManualTestActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        final Account[] accounts = mAccountManager.getAccountsByType(InsertCoins.ACCOUNT_TYPE_CMAccount);
    ...
    private void getToken() {
        mAccountManager.getAuthToken(mSelectedAccount, InsertCoins.AUTHTOKEN_TYPE_ACCESS, null, this, new AccountManagerCallback<Bundle>() {
            @Override
    ...
    private void invalidateAuthToken() {
        String token = mAccountManager.peekAuthToken(mSelectedAccount, InsertCoins.AUTHTOKEN_TYPE_ACCESS);
        if (token != null) {
            mAccountManager.invalidateAuthToken(InsertCoins.AUTHTOKEN_TYPE_ACCESS, token);
    ...
    private void expireAuthToken() {
        mAccountManager.setUserData(mSelectedAccount, InsertCoins.AUTHTOKEN_EXPIRES_IN, "0");
        String token = mAccountManager.peekAuthToken(mSelectedAccount, InsertCoins.AUTHTOKEN_TYPE_ACCESS);
        if (token != null) {
            mAccountManager.invalidateAuthToken(InsertCoins.AUTHTOKEN_TYPE_ACCESS, token);
        }
